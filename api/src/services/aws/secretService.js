 ...
 */
export async function getSecret(secretName, isJson = true) {
 ...
 * @param {boolean} isJson if true to a JSON.parse() on the results
 */
async function getSecretFromSecretsManager(secretName, isJson = true) {
  if (!client) {
    client = getClient();
  }
  ...
  if (response.SecretString !== undefined) {
    ...
    secretString = Buffer.from(response.SecretBinary, 'base64').toString('ascii'); // eslint-disable-line
  } else {
